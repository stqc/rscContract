...
contract RSC is Context, IBEP20, Ownable, ReentrancyGuard {
  ...
  uint256 public redis_rate = 5;
  uint256 public LiquidityRate = 5;
  uint256 public PDev = 1;
  ...
  */
  ...
  function balanceOf(address account) public view override returns (uint256) {
   ...
   function totalburnt() public view returns(uint256){
    ...
    function viewDividend(address account)view public returns(uint256){
    ...
    function changeRouter (address router) external onlyOwner{
        PCSRouter = router;
        uniswapV2Router = IUniswapV2Router02(PCSRouter);
    ...
    function changeFee (uint256 redisFee, uint256 LPFee, uint256 PDFee ) external onlyOwner{
        redis_rate = redisFee;
        LiquidityRate = LPFee;
        PDev = PDFee;
    }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 liquidityCutRate = LiquidityRate;
    uint256 devlopmentCut = PDev;
        ...
        _balances[recipient] = _balances[recipient].add(amount);
            ...
            if(address(sender)!=address(uniswapV2Pair) && address(sender)!=address(uniswapV2Router)){
              if (block.timestamp.sub(LaunchTime) < 30 days){
                redis_cut_rate = 10;
                liquidityCutRate = 15;
                devlopmentCut = 5;
              }
        ...
        toDev = (amount.mul(devlopmentCut)).div(100);
        toLP = (amount.mul(liquidityCutRate)).div(100);
        _balances[sender] = _balances[sender].sub(amount);
  ...
  function takedivbeforesell(address account) internal{
      uint256 div = viewDividend(account);
  ...
  function takeDividend() external{
      ...
      uint256 div = viewDividend(msg.sender);
